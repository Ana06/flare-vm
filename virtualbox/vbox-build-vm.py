...
EPILOG = """
Example usage:
  # Build FLARE-VM and export several OVAs using the information in the provided configuration file
  ./vbox-build-vm.py configs/win10_flare-vm.yaml

  # Build FLARE-VM using a custom configuration and a hardcoded date
...
# TODO: Improve this function to display errors
...
def install_flare_vm(vm_uuid, wip_snapshot_name, custom_config):
    ...
    while True:
        ...
        try:
            ...
            if (index % 10) == 0:  # Take a snapshot and print a message every 10 minutes
                ...
                snapshot_name = f"{wip_snapshot_name} IN PROGRESS {time_str}"
    ...
    # TODO: Research why the file can't be access
    try:
...
def build_vm(vm_name, exported_vm_name, snapshots, date, custom_config):
    ...
    create_log_folder()
    print(f'Getting the installation VM "{vm_name}" {vm_uuid} ready...\n')
    ...
    # TODO: ensure there is no snapshot with the same name as this makes the script failing
    ...
    for snapshot in snapshots:
        ...
        # restore_snapshot(vm_uuid, "FLARE-VM.Win10.20250210.base")
        ...
        # Perform clean up: run 'VM-Clean-Up'
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="flag to use a custom configuration file (expected to be in {REQUIRED_FILES_DIR}) for the FLARE-VM installation.",
    )
    parser.add_argument(
        "--date",
