...
def get_hostonlyif_name():
    ...
    # Example of `VBoxManage snapshot list hostonlyifs` output:
    # Name:            vboxnet0
...
def ensure_hostonlyif_exists():
    ...
    if not hostonlyif_name:
        ...
        hostonlyif_name = get_hostonlyif_name()
        if not hostonlyif_name:
            raise Exception("Failed to create new hostonly interface.")
...
def get_vm_state(vm_uuid):
    ...
    # Example of `VBoxManage showvminfo <VM_UUID> --machinereadable`:
    # VMState="poweroff"
...
def ensure_vm_running(vm_uuid):
    ...
    if not wait_until_vm_state(vm_uuid, "running"):
        raise TimeoutError(f"Unable to start VM {vm_uuid}.")
...
def ensure_vm_shutdown(vm_uuid):
    ...
    if not wait_until_vm_state(vm_uuid, "poweroff"):
        raise TimeoutError(f"Unable to shutdown VM {vm_uuid}.")
